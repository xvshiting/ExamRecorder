...
class KeyboardListenerQt(QObject):
    ...
    def _record_raw_keystroke(self, event, event_type):
        ...
        # 获取输入框内容
        input_content = self.controller.components['input_section'].input_box.toPlainText()
...
class KeyboardListenerPynputProcess:
    ...
    def _poll_keystrokes(self):
        ...
        start_time = getattr(self.controller.data_model, 'recording_start_time', None)
        input_content = self.controller.components['input_section'].input_box.toPlainText()
        for k in keystrokes:
            # 字段名统一
            k['key'] = k.pop('key_code', None)
            k['text'] = k.pop('key_text', None)
            if start_time is not None and 'absolute_timestamp' in k:
                k['timestamp'] = k['absolute_timestamp'] - start_time
            else:
                k['timestamp'] = k.get('absolute_timestamp', 0)
            k['input_content'] = input_content
            self.controller.add_raw_keystroke(k)